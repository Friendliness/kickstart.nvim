...
local sqlite_library = vim.fn.expand '~/.local/lib/libsqlite3.so'
if vim.uv.fs_stat(sqlite_library) then
  vim.g.sqlite_clib_path = sqlite_library
end
...
require('lazy').setup({
  ...
  -- FIXME: Sometimes seems to cause problems: python 'with' context managers
  ...
  { -- Adds git related signs to the gutter, as well as utilities for managing changes
    ...
    opts = {
      ...
      on_attach = function(bufnr)
        ...
        map('n', '<leader>hS', gs.stage_buffer, { desc = 'git hunk [S]tage buffer' })
        map('n', '<leader>hu', gs.undo_stage_hunk, { desc = 'git undo stage [hu]nk' })
        ...
        map('n', '<leader>tb', gs.toggle_current_line_blame, { desc = 'git [t]oggle line [b]lame' })
        map('n', '<leader>hd', gs.diffthis, { desc = 'git [h]unk [d]iffthis' })
        map('n', '<leader>hD', function()
          gs.diffthis '~'
        end, { desc = 'git [h]unk [D]iff' })
        map('n', '<leader>td', gs.toggle_deleted, { desc = 'git hunk [t]oggle [d]eleted' })
